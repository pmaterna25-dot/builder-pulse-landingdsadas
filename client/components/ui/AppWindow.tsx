...
type Item = {
  ...
  selectedSlots?: ("left" | "mid" | "right")[];
  umowaWystepujacaPrzy?: boolean;
  tags?: string[];
  umowaWystepujacaPrzyRef?: number | null;
  suAmount?: number;
};
...
export default function AppWindow({
  ...
  const [localItems, setLocalItems] = useState<Item[]>(() =>
    Array.from({ length: 40 }, () => ({
      label: "Za co odpowiada ten box — krótki opis.",
      description: "",
      link: "",
      fileName: "",
      color: "blue",
  ...
  const generateFromSelections = () => {
    const selected = items
      .map((it, idx) => ({ ...it, idx }))
      .filter((it) => it.selectedSlots && it.selectedSlots.length > 0);
    ...
    const parts = selected.map(
      ({ label, selectedSlots, description, link, fileName }) => {
  ...
  const savePackage = (name?: string) => {
    ...
    const pkgName = (
      name ||
      packageName ||
      `package_${new Date().toISOString().slice(0, 19).replace(/[:T]/g, "-")}`
    ).trim();
    const fileIds = items
      .filter(
        (it) =>
          it.selectedSlots && it.selectedSlots.includes("right") && it.fileId,
      )
    ...
    const invalid = sel.filter((i) => {
      const it = items[i];
      const hasLuxmed = (it.tags || []).some(
        (t) => (t || "").toLowerCase() === "luxmed",
      );
      ...
      const ref =
        typeof it.umowaWystepujacaPrzyRef === "number"
          ? it.umowaWystepujacaPrzyRef
          : null;
      if (ref !== null) {
    ...
    if (invalid.length > 0) {
      alert(
        `Nie można zapisać pakietu — pozycje (${invalid.map((i) => i + 1).join(", ")}) mają tag "luxmed" bez poprawnie ustawionej umowy podstawowej.`,
      );
      return;
    }
  ...
  const downloadPackageAsZip = async (pkg: SavedPackage) => {
    ...
    const invalid = (pkg.selectedIndices || []).filter((i) => {
      const it = items[i];
      const hasLuxmed =
        it && (it.tags || []).some((t) => (t || "").toLowerCase() === "luxmed");
    ...
    if (invalid.length > 0) {
      alert(
        `Nie można pobrać pakietu — pozycje (${invalid.map((i) => i + 1).join(", ")}) mają tag "luxmed" bez włączonej umowy podstawowej.`,
      );
      return;
    }
    ...
    try {
        ...
        JSON.stringify(
          {
            ...
            selectedIndices: pkg.selectedIndices,
          },
          null,
          2,
        ),
      ...
      const bundle = {
        meta: { name: pkg.name, createdAt: pkg.createdAt },
        generatedText: pkg.generatedText,
        files: pkg.fileIds.map((id) => savedFiles.find((f) => f.id === id)),
      };
  ...
  const exportToCSVAndSave = (filename?: string) => {
    ...
    const rows = items.map((it, idx) => [
      ...
      it.link || "",
      it.fileName || "",
      it.selectedSlots ? it.selectedSlots.join(";") : "",
    ]);
  ...
  const handleFileChange = (idx: number, file?: File | null) => {
    ...
    reader.onload = () => {
      ...
      const fileObj: SavedFile = {
        id: String(Date.now()) + "_" + file.name,
        name: file.name,
        dataUrl,
      ...
      <div className="w-full h-full bg-white p-6">
        <div className="flex h-full gap-6">
          <div className="w-[480px] flex flex-col gap-4 min-h-0">
            <div
              aria-hidden
              className="h-14 border-2 border-slate-800 bg-white"
            />
                          ...
                          {editingIndex === idx ? (
                            <>
                              ...
                              <div className="flex items-center gap-2 ml-3">
                                <button
                                  aria-label="green"
                                  onClick={() => setColor(idx, "green")}
                                  className="w-6 h-6 rounded-full bg-green-500 ring-2 ring-white shadow-sm"
                                />
                                <button
                                  aria-label="blue"
                                  onClick={() => setColor(idx, "blue")}
                                  className="w-6 h-6 rounded-full bg-blue-500 ring-2 ring-white shadow-sm"
                                />
                                <button
                                  aria-label="amber"
                                  onClick={() => setColor(idx, "amber")}
                                  className="w-6 h-6 rounded-full bg-amber-400 ring-2 ring-white shadow-sm"
                                />
                              </div>
                              <div className="ml-3 flex flex-col text-xs text-slate-700">
                                <label className="flex items-center gap-2">
                                  <input
                                    type="checkbox"
                                    checked={!!item.umowaWystepujacaPrzy}
                                    onChange={(e) =>
                                      handleChange(
                                        idx,
                                        "umowaWystepujacaPrzy",
                                        e.target.checked,
                                      )
                                    }
                                  />{" "}
                                  <span>umowa występująca przy</span>
                                </label>
                                {editingIndex === idx &&
                                item.umowaWystepujacaPrzy ? (
                                  <select
                                    value={item.umowaWystepujacaPrzyRef ?? ""}
                                    onChange={(e) => {
                                      const v =
                                        e.target.value === ""
                                          ? null
                                ...
                                {editingIndex === idx ? (
                                  <div className="mt-2">
                                    <div className="text-xs text-slate-600 mb-1">
                                      Dodatkowe informacje (oddzielaj
                                      przecinkami)
                                    </div>
                                    <input
                                      ...
                                      placeholder="np. luxmed, assistance, śmierć"
                            ...
                            <div className="text-xs text-slate-500">
                              {(
                                item.label ||
                                "Za co odpowiada ten box — krótki opis."
                              ).slice(0, 30)}
                            </div>
                          )}
                        </div>
                      ...
                      <div className="flex gap-3 mt-3 min-w-0">
                        ...
                        <div className="flex-1 min-w-0">
                          <div className="bg-white/80 rounded-lg shadow-md p-3 h-full flex flex-col items-center justify-center text-center">
                            <div
                              onMouseEnter={() => onCircleEnter(idx, "left")}
                              onMouseLeave={onCircleLeave}
                              onClick={() => onCircleClick(idx, "left")}
                              className={`${items[idx].selectedSlots && items[idx].selectedSlots.includes("left") && mode === "home" ? "rainbow-circle text-white" : "bg-blue-500"} rounded-full p-2 mb-2 inline-flex ${hoveredKey === `${idx}-left` ? "scale-105 shadow-lg" : ""}`}
                            >
                              <svg
                                xmlns="http://www.w3.org/2000/svg"
                              ...
                              >
                                <path
                                  ...
                                  strokeLinejoin="round"
                                  strokeWidth={2}
                                  d="M8 7V3m8 4V3m-9 8h10M5 21h14a2 2 0 002-2V7a2 2 0 00-2-2h-3l-2-2H9L7 5H4a2 2 0 00-2 2v12a2 2 0 002 2z"
                                />
                              </svg>
                            </div>
                            {editingIndex === idx ? (
                              ...
                              <>
                                ...
                                <div className="text-xs text-slate-500 mt-1">
                                  {truncatePreview(item.description, 8) ||
                                    "opis umowy"}
                                </div>
                              </>
                            )}
                        ...
                        <div className="flex-1 min-w-0">
                          <div className="bg-white/80 rounded-lg shadow-md p-3 h-full flex flex-col items-center justify-center text-center">
                            <div
                              onMouseEnter={() => onCircleEnter(idx, "mid")}
                              onMouseLeave={onCircleLeave}
                              onClick={() => onCircleClick(idx, "mid")}
                              className={`${items[idx].selectedSlots && items[idx].selectedSlots.includes("mid") && mode === "home" ? "rainbow-circle text-white" : "bg-green-500"} rounded-full p-2 mb-2 inline-flex ${hoveredKey === `${idx}-mid` ? "scale-105 shadow-lg" : ""}`}
                            >
                              <svg
                                xmlns="http://www.w3.org/2000/svg"
                                className="h-5 w-5 text-white"
                              ...
                              >
                                <path
                                  ...
                                  strokeLinejoin="round"
                                  strokeWidth={2}
                                  d="M13.828 14.828a4 4 0 010-5.656L15 7.999m-6 8l1.172-1.172a4 4 0 005.656 0L19 13.657"
                                />
                              </svg>
                            </div>
                            {editingIndex === idx ? (
                        ...
                        <div className="flex-1 min-w-0">
                          <div className="bg-white/80 rounded-lg shadow-md p-3 h-full flex flex-col items-center justify-center text-center">
                            <label
                              onMouseEnter={() => onCircleEnter(idx)}
                              onMouseLeave={onCircleLeave}
                            ...
                            >
                              ...
                              />
                              <div
                                onClick={() => onCircleClick(idx, "right")}
                                className={`${items[idx].selectedSlots && items[idx].selectedSlots.includes("right") && mode === "home" ? "rainbow-circle text-white" : "bg-amber-400"} rounded-full p-2 mb-2 inline-flex ${hoveredKey === `${idx}-right` ? "scale-105 shadow-lg" : ""}`}
                              >
                                <svg
                                  xmlns="http://www.w3.org/2000/svg"
                                  className="h-5 w-5 text-white"
                                ...
                                >
                                  <path
                                    ...
                                    strokeLinejoin="round"
                                    strokeWidth={2}
                                    d="M4 16v1a3 3 0 003 3h10a3 3 0 003-3v-1M12 12v9m0-9l3 3m-3-3L9 15"
                                  />
                                </svg>
                              </div>
                            </label>
                        ...
                        <div className="absolute bottom-12 right-3 bg-white border rounded shadow p-3 w-40 text-center">
                          ...
                          <div className="flex justify-between gap-2">
                            ...
                            <button
                              ...
                              className="flex-1 px-2 py-1 bg-slate-200 rounded"
                            >
                              Nie
                            </button>
                          </div>
          ...
          {filtered.length > 0 ? (
            <div className="flex-1 border-2 border-slate-800 bg-white min-w-0 p-4 flex flex-col">
              ...
              {(() => {
                const selPos = items
                  .map((it, i) =>
                    it.selectedSlots && it.selectedSlots.length > 0
                      ? i + 1
                      : null,
                  )
                  .filter(Boolean) as number[];
                const special = selPos
                  .filter((n) => n >= 1 && n <= 3)
                  .sort((a, b) => b - a);
                if (special.length > 0) {
                  ...
                  return (
                    <div className="mb-3 pt-3 pr-3 pl-3 pb-1.5 bg-yellow-50 border-l-4 border-amber-400 text-slate-800">
                      ...
                      <div className="text-sm mt-1">
                        Wybrane umowy ({list}) będą sumowane przy wyliczaniu
                        świadczenia SU.
                      </div>
                      ...
                      {special.includes(3) ? (
                        <div className="mt-2 text-sm">
                          ...
                          <div>
                            Wystąpienie śmierci z tytułu wypadku komunikacyjnego
                            powoduje, że automatycznie jest aktywowana umowa "
                            {items[2]?.label || "Pozycja 3"}" oraz umowa główna
                            "{items[0]?.label || "Pozycja 1"}" co daje
                            zwielokrotnienie kwoty.
                          </div>
                        </div>
                      ) : null}
                    </div>
              ...
              <div className="mb-3 overflow-auto max-h-40">
                {generatedText ? (
                  ...
                  <div className="text-xs text-slate-500">
                    Brak podglądu — wygeneruj zawartość.
                  </div>
                )}
              </div>
                ...
                <div className="mt-3 border-t pt-3">
                  ...
                  <div className="mb-2">
                    <input
                      ...
                      placeholder="Nazwa pakietu (np. Pawelmaterna)"
                      className="w-full border rounded px-2 py-1 text-sm"
                    />
                      ...
                      {savedFiles.map((f) => (
                        ...
                        >
                          ...
                          <div className="flex items-center gap-2">
                            ...
                            <button
                              onClick={() => {
                                setGeneratedText(atob(f.dataUrl.split(",")[1]));
                                setFolderOpen(false);
                              }}
                      ...
                      <div className="flex flex-col gap-2">
                        {savedPackages.map((p) => (
                          ...
                          >
                            ...
                            <div className="flex items-center gap-2">
                              <button
                                onClick={() => downloadPackageAsZip(p)}
                                className="px-2 py-1 bg-green-600 text-white rounded text-sm"
                              >
                                Pobierz ZIP
                              </button>
                              <button
                                onClick={() => {
                                  setGeneratedText(p.generatedText);
                                  setFolderOpen(false);
                                }}
                                className="px-2 py-1 bg-slate-200 rounded text-sm"
                              >
