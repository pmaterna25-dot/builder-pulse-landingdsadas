...
type Item = { label: string; description: string; link: string; fileName: string; fileId?: string; color?: 'green' | 'blue' | 'amber'; selectedSlots?: ('left'|'mid'|'right')[]; umowaWystepujacaPrzy?: boolean; tags?: string[]; umowaWystepujacaPrzyRef?: number | null; suAmount?: number };
              ...
              {(mode === 'settings' || mode === 'home') ? (
                <div className="h-full overflow-y-auto pr-2 min-w-0">
                  {filtered.map(({item, idx}) => (
                    <div key={idx} className={`mb-3 border rounded p-4 min-h-[160px] ${bgFor(item.color)}`}>
                      <div className="flex flex-col items-center">
                        {mode === 'home' ? (
                          <div className="w-full flex justify-center mb-2">
                            <div className="flex items-center gap-2">
                              <label className="text-sm font-medium">Wpisz SU</label>
                              <input
                                type="number"
                                value={item.suAmount ?? ''}
